...
*/
/* SOLUTION
return an array => [the last number to determine divisibility, number of steps]
parameter number => double the last digit and subtract from number made of digits prior
continue until number is 2 digits and determine divisibility
*/
function seven(m) {
	let num = String(m),
		count = 0;
	while (num.length > 2) {
		count++;
		let numA = Number(num.slice(0, num.length - 1)),
			numB = Number(num.slice(num.length - 1));
		// double the last digit and subtract from number made of digits prior
		num = String(numA - numB * 2);
	}
